...
def event_manual_refocus(opt, event_path, aps_path, save_name):
    ## function used to manually refocus events and pack them to event frames
    ...
    reference_time = data.get('ref_t')  #timestamp at reference camera pose
    fx = data.get('fx') # from camera intrinsic matrix
    d = data.get('depth') #depth
    v = opt.v # camera speed (m/s) default: 0.177 
...
def pack_without_refocus(opt, event_path, aps_path, save_name):
    ## function used to pack unfocused events to event frames
    data = np.load(event_path,allow_pickle=True).item()
    eventData = data.get('events')  #unfocused events
    d = data.get('depth')
    fx = data.get('fx')
    ref_t = data.get('ref_t') 
...
if __name__ == '__main__':
    ...
    for i in range(len(input_event_name)):
        ...
        if opt.do_event_refocus == 1:
            event_manual_refocus(opt, current_event_path, current_aps_path, save_name)
        else:
